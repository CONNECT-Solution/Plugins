/**
 * FHIR Reference Client used with minor modifications from repo: https://github.com/cnanjo/FhirJavaReferenceClient
 *
...
/**
 * Helper class handling lower level HTTP transport concerns. TODO Document methods.
 *
...
public class ClientUtils {
    ...
    public static byte[] issueGetFeedRequestBinary(URI resourceUri, String feedFormat, HttpHost proxy) {
        HttpResponse response = issueGetRequest(resourceUri, feedFormat, proxy);
        
        if(response != null && response.getStatusLine().getStatusCode() != 200) {
            throw new EFhirClientException("Server returned error code " + response.getStatusLine().getStatusCode());
        }
        ...
        try {
            ...
            LOG.error("Unable to get binary from requets: " + ex.getMessage(), ex);
        }
    ...
    private static HttpResponse issueGetRequest(URI resourceUri, String feedFormat, HttpHost proxy) {
        HttpGet httpget = new HttpGet(resourceUri);
        configureFhirRequest(httpget, feedFormat);
        return sendRequest(httpget, proxy);
    }
    ...
    protected static void configureFhirRequest(HttpRequest request, String format, List<Header> headers) {
        ...
        if (format.equals(ResourceFormat.RESOURCE_JSON.getHeader())) {
            request.addHeader("Accept", FeedFormat.FEED_JSON.getHeader());
        } else if (format.equals(ResourceFormat.RESOURCE_XML.getHeader())) {
            request.addHeader("Accept", FeedFormat.FEED_XML.getHeader());
        } else if (format.equals(FeedFormat.FEED_JSON.getHeader())) {
            request.addHeader("Accept", ResourceFormat.RESOURCE_JSON.getHeader());
        } else if (format.equals(FeedFormat.FEED_XML.getHeader())) {
            request.addHeader("Accept", ResourceFormat.RESOURCE_XML.getHeader());
        }
